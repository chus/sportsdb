...
function positionStyle(p: number | null): string {
  ...
  if (p === 2) return "bg-surface-2 text-ink border-line";
            ...
            icon={
              <div className="relative w-16 h-16 bg-surface rounded-xl flex items-center justify-center flex-shrink-0 p-2">
                {team.logoUrl ? (
                  ...
                  <Flag className="w-8 h-8 text-faint" />
                )}
              </div>
            }
            badges={
              <div className="flex items-center gap-2 flex-wrap">
                {team.shortName && (
                  <span className="text-xs font-bold bg-surface/20 px-2.5 py-1 rounded-full tracking-wider">
                    {team.shortName}
                  </span>
                )}
                {team.coachName && (
                  ...
                  >
                    ...
                    <div className="space-y-0.5 text-xs">
                      <div className="flex items-baseline gap-1">
                        ...
                        <span className="text-muted">title{t.champions === 1 ? "" : "s"}</span>
                      </div>
                      {t.runnersUp > 0 && (
            ...
            <section>
              ...
              <div className="space-y-6">
                {tournamentSections.map(([key, entry]) => (
                  <div key={key} className="bg-surface rounded-xl border border-line overflow-hidden">
                    <div className="px-5 py-3 border-b border-line bg-surface-2 flex items-center justify-between">
                      ...
                      <span className="text-xs text-muted">{entry.rows.length} top-4 finish{entry.rows.length === 1 ? "" : "es"}</span>
                    </div>
                    <ul className="divide-y divide-line">
                      {entry.rows.map((r) => (
                        <li key={`${key}-${r.year}`} className="px-5 py-2.5 flex items-center gap-4 text-sm">
                          <span className="font-mono text-muted w-12">{r.year}</span>
                          <span
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  ...
                  .map((g) => (
                    <div key={g.title} className="bg-surface rounded-xl border border-line p-4">
                        ...
                        {g.group.slice(0, 8).map((p) => (
                          <li key={p.id}>
                            <PlayerLink
                              ...
                              className="flex items-center gap-2 text-sm text-ink hover:text-blue-600"
                            >
                              {p.knownAs || p.name}
                            </PlayerLink>
                          </li>
              ...
              <div className="bg-surface rounded-xl border border-line p-4">
                {nextMatch && (
                  <div className="mb-4 pb-4 border-b border-line">
                    <p className="text-xs font-semibold text-muted uppercase mb-1">Next</p>
                ...
                {recentMatches.length > 0 && (
                  <>
                    <p className="text-xs font-semibold text-muted uppercase mb-2">Recent</p>
