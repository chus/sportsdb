...
export default function AdminReferralsPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="h-8 w-8 animate-spin rounded-full border-4 border-line border-t-blue-600" />
      </div>
    );
  }

  if (!data) return <p className="py-10 text-center text-muted">Failed to load data</p>;
  ...
  return (
    <div>
      <h1 className="mb-8 text-2xl font-bold text-ink">Referral Analytics</h1>

      {/* Funnel */}
      <div className="mb-8 rounded-xl border border-line bg-surface p-6">
        <h2 className="mb-4 text-lg font-semibold text-ink">Referral Funnel</h2>
          ...
          {funnelStages.map((stage, i) => (
            <div key={stage.label} className="flex items-center gap-2 flex-1">
              <div className="flex-1 rounded-lg border border-line p-4 text-center">
                ...
                <p className="text-2xl font-bold text-ink">{stage.value}</p>
      ...
      {/* Top Referrers */}
      <div className="mb-8 rounded-xl border border-line bg-surface p-6">
        <h2 className="mb-4 text-lg font-semibold text-ink">Top Referrers</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-line text-left">
                  ...
                  <th className="pb-3 text-center font-medium text-muted">Signups</th>
                  <th className="pb-3 text-center font-medium text-muted">Subscriptions</th>
                  <th className="pb-3 text-center font-medium text-muted">Rewards</th>
                </tr>
              </thead>
              <tbody>
                {topReferrers.map((r) => (
                  <tr key={r.userId} className="border-b border-line">
                    ...
                    <td className="py-3 text-center font-semibold text-ink">{r.signups}</td>
                    <td className="py-3 text-center font-semibold text-ink">{r.subscriptions}</td>
                    <td className="py-3 text-center font-semibold text-ink">{r.rewards}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="rounded-xl border border-line bg-surface p-6">
        <h2 className="mb-4 text-lg font-semibold text-ink">Recent Events</h2>
