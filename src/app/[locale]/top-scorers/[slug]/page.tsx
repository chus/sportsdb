...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${competition.name} Top Scorers ${seasonLabel} – Goals & Stats`;
...
export default async function CompetitionTopScorersPage({ params }: PageProps) {
  ...
  return (
    <>
      ...
      {scorers.length > 0 && (
        <ItemListJsonLd
          name={`${competition.name} Top Scorers`}
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              {allSeasons.map((s) => (
                <Link
                  ...
                  className="px-3 py-1.5 rounded-full text-sm font-medium bg-surface border border-line text-ink hover:border-orange-300 hover:text-orange-600 transition-colors"
                >
                  {s}
                </Link>
              ))}
                  ...
                  <tbody className="divide-y divide-line">
                    {scorers.map(({ stat, player, team }, index) => (
                      <tr key={stat.id} className="hover:bg-surface-2 transition-colors">
                        ...
                        <td className="px-2 sm:px-4 py-3 text-center font-bold text-ink">{stat.goals}</td>
                        <td className="px-2 sm:px-4 py-3 text-center text-muted">{stat.assists}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
